...
AtomVecPeptide::AtomVecPeptide(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecPeptide::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecPeptide::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecPeptide::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecPeptide::write_data_hybrid(FILE *fp, double *buf)
{
